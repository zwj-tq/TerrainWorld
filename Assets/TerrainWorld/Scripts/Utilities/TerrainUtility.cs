...
namespace TWorld.Utility
{
    ...
    public static class TerrainUtility
    {
        ...
        public static Vector2Int GetHeightmapIndex(Terrain terrain, Vector3 point)
        {
            ...
            int z = (int)((point.z - terrain.GetPosition().z) / length * tData.heightmapResolution);
        ...
        public static Vector3 GetIndexWorldPoint(Terrain terrain, int x, int z, float y)
        {
            ...
            float _x = data.size.x / (data.heightmapResolution - 1) * x;
            float _z = data.size.z / (data.heightmapResolution - 1) * z;
